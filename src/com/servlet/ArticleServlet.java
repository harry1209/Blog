...
import com.dao.RestoreDao;
import com.form.ArticleForm;
import com.form.ArticleTypeForm;
import com.form.RestoreForm;
...
public class ArticleServlet extends HttpServlet {
	...
	public void updateArticle(HttpServletRequest request,
		...
		if (articleDao.operationArticle("修改", articleForm)) {
			out
					.print("<script language=javascript>alert('修改日志成功，请重新查询！');window.location.href='back_ArticleSelect.jsp';</script>");
		} else {
			out
					.print("<script language=javascript>alert('修改日志失败！');history.go(-1);</script>");
		}
	}
	...
	public void deleteArticle(HttpServletRequest request,
		...
		if (articleDao.operationArticle("删除", articleForm)) {
			out
					.print("<script language=javascript>alert('删除日志成功，请重新查询！');window.location.href='back_ArticleSelect.jsp';</script>");
		} else {
			out
					.print("<script language=javascript>alert('删除日志失败！');history.go(-1);</script>");
		}
	}
	...
	public void deleteArticleType(HttpServletRequest request,
		...
		if (articleTypeDao.operationArticleType("删除", ArticleTypeForm)) {
			out
					.print("<script language=javascript>alert('删除日志类别成功，请重新查询！');window.location.href='back_ArticleTypeSelect.jsp';</script>");
		} else {
			out
					.print("<script language=javascript>alert('您需要将类别所在的日志删除,才可删除此类别！');history.go(-1);</script>");
		}
	...
	public void addArticleType(HttpServletRequest request,
		...
		if (articleTypeDao.operationArticleType("添加", ArticleTypeForm)) {
			out
					.print("<script language=javascript>alert('添加日志类别成功，请重新查询！');window.location.href='back_ArticleTypeSelect.jsp';</script>");
		} else {
			out
					.print("<script language=javascript>alert('添加日志类别失败！');history.go(-1);</script>");
		}
