...
public class ConsumerServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (method == 2) {
			queryConsumerForm(request, response);// 对用户进行查询
		}
		if (method == 3) {
			deleteConsumerForm(request, response);// 对用户进行删除操作
		}
		if (method == 4) {
			queryConsumerHostForm(request, response); // 对主人的查询操作
		}
		if (method == 5) {
			updateConsumerHostForm(request, response); // 对主人信息的修改操作
		}
		if (method == 6) {
			front_updateConsumerForm(request, response); // 用户对登录用进行修改
		}
	...
	public void queryConsumerHostForm(HttpServletRequest request,
			HttpServletResponse response) throws ServletException, IOException {
		consumerDao = new ConsumerDao();
		request.setAttribute("form", consumerDao.getConsumerForm("xp"));
		RequestDispatcher requestDispatcher = request
				.getRequestDispatcher("back_consumerSelectHostForm.jsp");
		requestDispatcher.forward(request, response);
